...
function showPizza(id, img, name, price) {
    const countDiv = document.getElementById('countDiv');
    const obj = {
        id,
        img,
...
function addBasket() {
    ...
    basket.forEach(item => {
        ...
        basketContent.innerHTML += `
            <div class="basket-item flex justify-between mb-[10px]">
                <img src="${item.img}" alt="${item.name}" class="w-[40%] h-[50px] object-cover">
                <div class="w-[50%]">
    ...
    totalPriceElement.innerHTML = `Umumi mebleg: ${totalPrice.toFixed(2)} m`
